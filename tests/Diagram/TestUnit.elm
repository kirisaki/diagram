...
import Diagram.Unit exposing (Length(..), Unit(..), toLength, toString)
import Parser
...
suiteToString : Test
suiteToString =
    describe "toString inverse check" <|
        List.map generateTestCase
        [ ( "normal case", Diagram.Unit.toString <| Length 50 Percent, Length 50 Percent)
        , ( "float value", Diagram.Unit.toString <| Length 0.3 Vmax, Length 0.3 Vmax )
        ]
